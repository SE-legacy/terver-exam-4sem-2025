...
4. Если существует $M abs(xi^n) < infinity$, то $phi(t)$ имеет первые $n$ непрерывных производных и
	$ phi^((n))(0) = i^n M xi^n . $
...
Фиксируем найденное $A$, воспользуемся равномерной непрерывностью функции $e^(i (t_1 - t_2) x)$ на ограниченном множестве $abs(x) <= A$ (по теореме Кантора), и выберем такое $delta = delta(epsilon)$, что
$
...
$
abs(phi(t_1) - phi(t_2)) <= epsilon/2 space.quad P{abs(x) <= A} + epsilon/2 <= epsilon
$
...
Если существует $M abs(xi^n) < infinity$, то $phi(t)$ имеет первые $n$ непрерывных производных и
$ phi^((n))(0) = i^n M xi^n . $
